<template>
  <!-- As a page: the breadcrumb back to the list. As a drawer: close, the
       neighbours, and where this one sits in the list — the list is still
       there behind it, so the header does not need to name it. -->
  <header
    class="flex h-[52px] flex-shrink-0 items-center gap-3 border-b border-line px-4 md:h-[56px] md:px-5"
  >
    <template v-if="variant === 'drawer'">
      <button
        type="button"
        class="flex h-[30px] w-[30px] flex-none items-center justify-center rounded-md border border-line text-ink-2 transition-colors hover:border-ink-4"
        :aria-label="t('common.close')"
        @click="closeAfterSaving"
      >
        <svg
          class="h-[15px] w-[15px]"
          viewBox="0 0 24 24"
      ...
      <div class="flex flex-none items-center gap-[5px]">
        <button
          type="button"
          class="flex h-[30px] w-[30px] items-center justify-center rounded-md border border-line transition-colors disabled:text-ink-4 enabled:text-ink-2 enabled:hover:border-ink-4"
          :disabled="!hasPrev"
          :aria-label="t('chats.detail.previous')"
          @click="$emit('prev')"
        >
          <svg
            class="h-[15px] w-[15px]"
        ...
        <button
          ...
          class="flex h-[30px] w-[30px] items-center justify-center rounded-md border border-line transition-colors disabled:text-ink-4 enabled:text-ink-2 enabled:hover:border-ink-4"
          :disabled="!hasNext"
          :aria-label="t('chats.detail.next')"
          @click="$emit('next')"
        >
          <svg
            class="h-[15px] w-[15px]"
...
// The drawer lives at /chats/:id as well, so every composable's
// `route.params.id` already names the conversation on screen.
...
// Closing the drawer with an edit open should neither lose the work nor
// raise a confirm box, so it commits what is open and then closes. The
// markdown and list editors keep their explicit save: silently committing a
// half-written body is worse than asking.
const closeAfterSaving = async () => {
  if (editingTitle.value && !savingTitle.value) {
    try {
      await saveTitle()
    } catch (error) {
      console.error('Failed to save the title before closing:', error)
    }
  }
  emit('close')
}

// Escape is the same gesture as the close button, so it takes the same path.
defineExpose({ requestClose: closeAfterSaving })
